...
public class GqlKeyspaceSchema {
    ...
    private GraphQLInputType mutationOptions = GraphQLInputObjectType.newInputObject().name("MutationOptions")
            .field(GraphQLInputObjectField.newInputObjectField()
                    .name("consistency")
                    .type(GraphQLEnumType.newEnum().name("MutationConsistency")
                            .value("LOCAL_ONE")
                            .value("LOCAL_QUORUM")
                            .value("ALL")
                            .build())
    ...
    public GraphQLSchema.Builder build() {
        ...
        {
            GraphQLFieldDefinition emptyQueryField = GraphQLFieldDefinition.newFieldDefinition()
        ...
        {
            GraphQLFieldDefinition emptyMutationField = GraphQLFieldDefinition.newFieldDefinition()
    ...
    private List<GraphQLFieldDefinition> buildQuery(Table table) {
        GraphQLFieldDefinition query = GraphQLFieldDefinition.newFieldDefinition()
                .name(CaseUtil.toLowerCamel(table.name()))
                .argument(GraphQLArgument.newArgument()
                        .name("value")
                        .type(new GraphQLTypeReference(nameMapping.getEntityName().get(table) + "Input")))
                .argument(GraphQLArgument.newArgument()
                        .name("filter")
                        .type(new GraphQLTypeReference(nameMapping.getEntityName().get(table) + "FilterInput")))
                .argument(GraphQLArgument.newArgument()
                        .name("orderBy")
                        .type(new GraphQLList(new GraphQLTypeReference(nameMapping.getEntityName().get(table) + "Order"))))
                .argument(GraphQLArgument.newArgument()
                        .name("options")
        ...
        GraphQLFieldDefinition filterQuery = GraphQLFieldDefinition.newFieldDefinition()
    ...
    private GraphQLFieldDefinition buildUpdate(Table table) {
        return GraphQLFieldDefinition.newFieldDefinition()
                ...
                .argument(GraphQLArgument.newArgument()
                        .name("value")
                        .type(new GraphQLNonNull(new GraphQLTypeReference(nameMapping.getEntityName().get(table) + "Input"))))
                .argument(GraphQLArgument.newArgument()
                        .name("ifExists")
                        .type(Scalars.GraphQLBoolean))
                .argument(GraphQLArgument.newArgument()
    ...
    private Map<Column.ColumnType, GraphQLInputObjectType> buildFilterInputTypes() {
        ...
        GraphQLInputObjectType timestampFilterInput = filterInputType(CustomScalar.TIMESTAMP.getGraphQLScalar());
        GraphQLInputObjectType timeUUIDFilterInput = filterInputType(CustomScalar.TIMEUUID.getGraphQLScalar());
        GraphQLInputObjectType inetFilterInput = filterInputType(CustomScalar.INET.getGraphQLScalar());
        GraphQLInputObjectType bigIntFilterInput = filterInputType(CustomScalar.BIGINT.getGraphQLScalar());
        GraphQLInputObjectType decimalFilterInput = filterInputType(CustomScalar.DECIMAL.getGraphQLScalar());
        GraphQLInputObjectType varintFilterInput = filterInputType(CustomScalar.VARINT.getGraphQLScalar());
    ...
    private List<GraphQLInputObjectField> buildFilterInputFields(Table table) {
        ...
        Preconditions.checkState(!fields.isEmpty(), "Could not generate an input type for table, skipping.");
    ...
    private GraphQLType buildQueryOptionsInputType() {
        return GraphQLInputObjectType.newInputObject().name("QueryOptions")
                .field(GraphQLInputObjectField.newInputObjectField()
                        .name("consistency")
                        .type(GraphQLEnumType.newEnum().name("QueryConsistency")
                            .value("LOCAL_ONE")
                            .value("LOCAL_QUORUM")
                            .value("ALL")
                            .value("SERIAL")
    ...
    private GraphQLType buildOrderType(Table table) {
        GraphQLEnumType.Builder input = GraphQLEnumType.newEnum()
                .name(nameMapping.getEntityName().get(table) + "Order");
        for (Column columnMetadata : table.columns()) {
    ...
    private GraphQLType buildInputType(Table table) {
        GraphQLInputObjectType.Builder input = GraphQLInputObjectType.newInputObject()
    ...
    private GraphQLOutputType buildEntityResultOutput(Table table) {
        ...
        GraphQLOutputType entityResultType = GraphQLObjectType.newObject().name(nameMapping.getEntityName().get(table) + "Result")
                .field(GraphQLFieldDefinition.newFieldDefinition()
                        .name("pageState")
                        .type(GraphQLString))
                .field(GraphQLFieldDefinition.newFieldDefinition()
                        .name("values")
                        .type(new GraphQLList(new GraphQLNonNull(new GraphQLTypeReference(nameMapping.getEntityName().get(table))))))
                .build();
    ...
    public GraphQLObjectType buildType(Table table) {
        GraphQLObjectType.Builder builder = GraphQLObjectType.newObject().name(nameMapping.getEntityName().get(table));
        for (Column columnMetadata : table.columns()) {
            try {
    ...
    private GraphQLFieldDefinition.Builder buildOutputField(Table table, Column columnMetadata) {
        return new GraphQLFieldDefinition.Builder().name(nameMapping.getColumnName(table).get(columnMetadata))

...
public class CreateTableDataFetcher implements io.stargate.graphql.fetchers.SchemaFetcher, DataFetcher {
    ...
    public String getQuery(DataFetchingEnvironment dataFetchingEnvironment) {
        ...
        List<Map<String, Object>> clusteringKeys = dataFetchingEnvironment.getArgument("clusteringKeys");
        ...
        if (clusteringKeys != null) {
            for (Map<String, Object> key : clusteringKeys) {
                if (options != null) {
                    options = table.withClusteringOrder((String) key.get("name"), decodeClusteringOrder((String) key.get("order")));
                } else {
                    options = options.withClusteringOrder((String) key.get("name"), decodeClusteringOrder((String) key.get("order")));
                }
            }
        }
        String query;

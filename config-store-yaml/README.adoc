...
in the `/etc/config/stargate-config.yaml` location. Please note that the name of the actual YAML file is `stargate-config.yaml`
...
it will throw the `MissingExtensionSettingsException`. This is a `RuntimeException` and it's the caller's responsibility to catch and handle gracefully or propagate upper in the call stack.
...
=== Config Store YAML
Finally, when it is mounted into a file system, the `ConfigStoreActivator` can locate the file under: `/etc/config/stargate-config.yaml`

...
public class RestApiv2SchemaTest extends BaseRestApiTest {
  ...
  public void tableUpdateSimple() throws IOException {
    ...
    // Let's also verify that TTL is updated (related to [stargaate#1836])
    String body =
            RestUtils.get(
                    authToken,
                    String.format("%s/v2/schemas/keyspaces/%s/tables/%s?raw=true",
                            restUrlBase, keyspaceName, tableName),
                    HttpStatus.SC_OK);
    ...
    // 09-Jun-2022, tatu: Alas due to [stargate#1881] it is not yet exposed

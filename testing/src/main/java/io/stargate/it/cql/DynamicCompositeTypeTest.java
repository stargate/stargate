...
public class DynamicCompositeTypeTest extends BaseIntegrationTest {
  ...
  public void initTable(CqlSession session) {
    session.execute(
        "CREATE TABLE dynamic_comp_ks.dynamic_composite_table (\n"
            + "       k int PRIMARY KEY,\n"
  ...
  public void cleanupTable(CqlSession session) {
    session.execute("DROP TABLE dynamic_comp_ks.dynamic_composite_table IF EXISTS");
  }

  @Test
  @DisplayName("Try to create Table with DynamicCompositeType, insert and read")
  public void createDynamicCompositeTypeTable(CqlSession session) {
    ...
    row =
        session
            .execute("select k,stuff,dct from dynamic_comp_ks.dynamic_composite_table where k=123")
            .one();
    assertThat(row.getInt("k")).isEqualTo(123);
    assertThat(row.getString("stuff")).isEqualTo("text value");

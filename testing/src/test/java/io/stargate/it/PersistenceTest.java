...
public class PersistenceTest extends BaseOsgiIntegrationTest
{
    ...
    private final static int CUSTOM_PAGE_SIZE = 50;
    ...
    {
        createKeyspace();
        ResultSet createTable = dataStore.query().create().table(keyspace, table)
        ...
        assertThat(t).isEqualToComparingFieldByField(
                Schema.build().keyspace(keyspace).table(table)
                        .column("created", Boolean, PartitionKey)
                        .build().keyspace(keyspace).table(table));

        ResultSet alterTable = dataStore.query().alter().table(keyspace, table)
                .addColumn("added", Boolean)
                .execute();
        // TODO: [doug] 2020-07-10, Fri, 16:50 More schemaAgreement to revisit
//        assertThat(alterTable.waitedForSchemaAgreement()).isTrue();
        assertThat(dataStore.schema().keyspace(keyspace).table(table)).isEqualToComparingFieldByField(
                Schema.build().keyspace(keyspace).table(table)
                        .column("created", Boolean, PartitionKey)
...
//        assertThat(dropTable.waitedForSchemaAgreement()).isTrue();
    ...
    {
        createKeyspace();
        dataStore.query().create().table(keyspace, table)
        ...
        assertThat(dataStore.schema().keyspace(keyspace).table(table)).isEqualToComparingFieldByField(
                Schema.build().keyspace(keyspace).table(table)
                        ...
                        .column("R2", Varchar)
                        .column("S1", Varchar, Static)
                        .column("S2", Varchar, Static).build().keyspace(keyspace).table(table));
    }
...
//        Polygon polygon = new Polygon(new Point(30, 10), new Point(10, 20), new Point(20, 40), new Point(40, 40));
//        LineString lineString = new LineString(new Point(30, 10), new Point(40, 40), new Point(20, 40));

        // TODO: [doug] 2020-07-13, Mon, 9:26 Having trouble with UDTs, removing for now
//        UserDefinedType udtType = ImmutableUserDefinedType.builder().name("My_udt").keyspace(ks.name())
//                .addColumns(Column.create("a", Int), Column.create("b", Int), Column.create("c", Varchar)).build();

        List<Pair<Column.ColumnType, Object>> values = ImmutableList.<Pair<Column.ColumnType, Object>> builder()
                // Non frozen types
                ...
                .add(Pair.with(List.of(Duration), Arrays.asList(CqlDuration.newInstance(2, 3, 5),
                        CqlDuration.newInstance(2, 3, 6))))
                .add(Pair.with(Map.of(Varchar, Duration),
                        ImmutableMap.of("Alice", CqlDuration.newInstance(2, 3, 5),
                                "Bob", CqlDuration.newInstance(2, 3, 6))))
                ...
                .add(Pair.with(tupleTypeWithDuration,
                        tupleTypeWithDuration.create("Test",
                                nestedDuration.create(2, CqlDuration.newInstance(2, 3, 6)))))
...
//                .add(Pair.with(Map.of(Point, Polygon), ImmutableMap.of(point, polygon)))
//                .add(Pair.with(Map.of(LineString, Polygon), ImmutableMap.of(lineString, polygon)))
//                .add(Pair.with(Set.of(Point), ImmutableSet.of(point, new Point(3, 3), new Point(100, 100))))
//                .add(Pair.with(Set.of(Polygon), ImmutableSet.of(polygon, polygon, polygon)))
//                .add(Pair.with(Set.of(LineString), ImmutableSet.of(lineString, lineString, lineString)))
//                .add(Pair.with(List.of(Point), ImmutableList.of(point, new Point(-100, -200), new Point(3.0, 7.0))))
//                .add(Pair.with(List.of(Polygon), ImmutableList.of(polygon, polygon, polygon)))
//                .add(Pair.with(List.of(LineString), ImmutableList.of(lineString, lineString, lineString)))
//                .add(Pair.with(tupleTypeWithGeo, tupleTypeWithGeo.create(point, nestedGeo.create(polygon, lineString))))
                // TODO: [doug] 2020-07-13, Mon, 9:26 Having trouble with UDTs, removing for now
//                .add(Pair.with(udtType, udtType.create(23, 45, "some text")))
        ...
        Supplier<Stream<Column.ColumnType>> distinctColumnTypesStream = () -> values.stream().map(Pair::getValue0)
                .distinct();
...
//        dataStore.query().create().type(keyspace, udtType).ifNotExists().execute();
        dataStore.query().create().table(keyspace, table).column("PK", Text, PartitionKey)
                .column(distinctColumnTypesStream.get()
        ...
        distinctColumnTypesStream.get()
                .forEach(columnType ->
                {
                    assertThat(dataStore.schema().keyspace(keyspace).table(table).column(columnName(columnType)))
                            .withFailMessage("Table did not contain definition for column %s",
                                    columnType.cqlDefinition())
                            .isNotNull();
                    assertThat(dataStore.schema().keyspace(keyspace).table(table).column(columnName(columnType)).type())
        ...
        for (int i = 0; i < values.size(); i++)
        {
            ...
            dataStore.query()
                    .insertInto(keyspace, table).value("PK", "PK" + i)
                    .value(Value.create(columnName(pair.getValue0()), pair.getValue1()))
                    .execute();

            Row row = dataStore.query().select().star().from(keyspace, table).where("PK", Eq, "PK" + i)
                    .execute().one();
    ...
    /**
     * This will make sure that we can insert data and use it in a SELECT * FROM ks.tbl query. Under the hood this will
     * test every type's toInternalValue() / toExternalValue() methods.
     */
    @Test
    public void testUseTypesInWhere() throws Exception
    {
        createKeyspace();
        Map<Column.ColumnType, Object> values = ImmutableMap.<Column.ColumnType, Object> builder()
                .put(Ascii, "hi")
                .put(Bigint, 23L)
                ...
                .put(Double, 3.8d)
                // .put(Duration, com.datastax.driver.core.Duration.newInstance(2, 3, 5)) // can't be used as PK
                .put(Float, 4.9f)
...
//                .put(Polygon, new Polygon(new Point(30, 10), new Point(10, 20), new Point(20, 40), new Point(40, 40)))
//                .put(LineString, new LineString(new Point(30, 10), new Point(40, 40), new Point(20, 40)))
                .build();
        ...
        values.forEach((k, v) ->
        {
            try
            {
                ...
                Row row = dataStore.query().select().star().from(keyspace, tbl).where(c, Eq, v).execute().one();
                assertThat(row).isNotNull();
    ...
    {
        createKeyspace();
        dataStore.query().create().table(keyspace, table).column("a", Int, PartitionKey).column("b", Varchar)
                .column("c", Uuid)
                .execute();
        dataStore.query().create().index("byB").ifNotExists().on(keyspace, table).column("b").execute();

        assertThat(dataStore.schema().keyspace(keyspace).table(this.table)).isEqualToComparingFieldByField(
                Schema.build().keyspace(keyspace).table(table)
                        .column("a", Int, PartitionKey)
                        .column("b", Varchar)
                        .column("c", Uuid)
                        .secondaryIndex("byB").column("b")
                        .build().keyspace(keyspace).table(table));

        dataStore.query().create().index("byC").ifNotExists().on(keyspace, table).column("c").execute();

        assertThat(dataStore.schema().keyspace(keyspace).table(this.table).toString()).isEqualTo(
                Schema.build().keyspace(keyspace).table(table)
                        .column("a", Int, PartitionKey)
                        .column("b", Varchar)
                        .column("c", Uuid)
                        .secondaryIndex("byB").column("b")
                        .secondaryIndex("byC").column("c")
                        .build().keyspace(keyspace).table(table).toString());
    }
    ...
    public void testTupleWithAllSimpleTypes() throws UnknownHostException, ExecutionException, InterruptedException
    {
        ...
        typeToValue.forEach((type, value) -> columns.add(Column.create(type.name().toLowerCase(), type)));

        Keyspace ks = createKeyspace();
        ParameterizedType.TupleType tupleType = ImmutableTupleType.builder().addAllParameters(typeToValue.keySet())
                .build();

        dataStore.query().create()
                .table(ks.name(), table)
                .ifNotExists()
        ...
        dataStore.query().insertInto(ks.name(), table)
                .value("x", 1)
                .value("my_tuple", tupleType.create(actualValues))
                .execute();

        java.util.List<Row> rows = dataStore.query().select().star().from(ks.name(), table).where("x", Eq, 1).execute()
                .rows();
    ...
    public void testUDTWithAllSimpleTypes() throws UnknownHostException, ExecutionException, InterruptedException
    {
        ...
        UserDefinedType udtType = ImmutableUserDefinedType.builder().name(typeName).keyspace(ks.name())
        ...
        java.util.List<Row> rows = dataStore.query().select().star().from(ks.name(), table).where("x", Eq, 1).execute()
                .rows();
        assertThat(rows).isNotEmpty();
        // TODO: [doug] 2020-07-10, Fri, 16:43 we currently have a mismatch where some things are using UdtValue and others want UDTValue
//        Row row = rows.get(0);
//        UDTValue udtValue = row.getUDT("udt");
//        assertThat(udtValue).isNotNull();
//        columns.forEach(c -> assertThat(udtValue.get(c.name().toLowerCase(), Objects.requireNonNull(c.type()).codec()))
//                .isEqualTo(typeToValue.get(c.type())));
    ...
    private ImmutableMap<Column.ColumnType, Object> getSimpleTypesWithValues() throws UnknownHostException
    {
        return ImmutableMap.<Column.ColumnType, Object> builder()
                ...
                .put(Varint, BigInteger.valueOf(23))
//                .put(Point, new Point(3.3, 4.4))
//                .put(Polygon, new Polygon(new Point(30, 10), new Point(10, 20), new Point(20, 40), new Point(40,
//                        40)))
//                .put(LineString, new LineString(new Point(30, 10), new Point(40, 40), new Point(20, 40)))
                .build();
    }
    ...
    public void testUDT() throws ExecutionException, InterruptedException
    {
        ...
        String typeName = "my_type";
        UserDefinedType udtType = ImmutableUserDefinedType.builder().name(typeName).keyspace(ks.name())
                .addColumns(Column.create("a", Int),
        ...
        dataStore.query().insertInto(ks.name(), table)
                .value("x", 1)
                .value("udt", ks.userDefinedType(typeName)
                        .create(23, list, set, map, tuple))
                .execute();
...
//        assertThat(udtValue.getTupleValue("mytuple")).isEqualTo(tuple);
    ...
    public void testMvIndexes() throws ExecutionException, InterruptedException
    {
        ...
        dataStore.query().create().materializedView(keyspace, "byB").asSelect().star().column("b", PartitionKey)
                .column("a", Clustering, Desc).from(keyspace, table).execute();

        assertThat(dataStore.schema().keyspace(keyspace).table(this.table)).isEqualTo(
                Schema.build().keyspace(keyspace).table(table)
                        .column("a", Int, PartitionKey)
                        .column("b", Varchar)
                        .column("c", Uuid)
                        .materializedView("byB").column("b", PartitionKey).column("a", Clustering, Desc).column("c")
                        .build().keyspace(keyspace).table(table));

        dataStore.query().create().materializedView(keyspace, "byC").asSelect().star().column("c", PartitionKey)
                .column("a", Clustering, Asc).from(keyspace, table).execute();
        ...
        assertThat(dataStore.schema().keyspace(keyspace).table(this.table)).isEqualTo(
                Schema.build().keyspace(keyspace).table(table)
                        ...
                        .column("c", Uuid)
                        .materializedView("byB").column("b", PartitionKey).column("a", Clustering, Desc).column("c")
                        .materializedView("byC").column("c", PartitionKey).column("a", Clustering, Asc).column("b")
                        .build().keyspace(keyspace).table(table));
    }
...
//        int pageSize = storeName.equals("internalWithPaging") ? CUSTOM_PAGE_SIZE : DataStore.DEFAULT_ROWS_PER_PAGE;
    ...
    private void iterateOverResults(ResultSet resultSet, Iterator<Row> it, Set<Integer> knownValues,
                                    int pageSize, int totalResultSize)
    {
    ...
    public void testWithUnsetParameter() throws ExecutionException, InterruptedException
    {
        ...
        dataStore.query(String.format("insert into %s.%s (a,b) values (?,?)", tbl.cqlKeyspace(), tbl.cqlName()), 23,
                Parameter.UNSET)
                .get();
    ...
    {
        createKeyspace();
        dataStore.query().create().table(keyspace, table).column("graph", Boolean, PartitionKey).column("name", Column.Type.Text)
                .execute();
        dataStore.query().insertInto(keyspace, table).value("graph", true).value("name", "Bob").execute();
        ...
        dataStore.query().update(keyspace, table).value("name", null).where("graph", Eq, true).execute();
    ...
    public void testPrepStmtCacheInvalidation() throws ExecutionException, InterruptedException
    {
        ...
        dataStore.query().insertInto(keyspace, table)
                .value("graph", true)
                .value("name", "Bob")
                .execute();
        ...
        // afterwards we should be able to re-create the same schema and insert the same data without a problem
        ...
        // org.apache.cassandra.exceptions.UnknownTableException: Cannot find table, it may have been dropped
        dataStore.query().insertInto(keyspace, table)
                .value("graph", true)
                .value("name", "Bob")
                .execute();
    ...
    public void testKeyspaceReplicationAndDurableWrites() throws ExecutionException, InterruptedException
    {
        createKeyspace();
        Keyspace ks = dataStore.schema().keyspace(keyspace);
        assertThat(ks.replication()).isEqualTo(
    ...
    public void testUpdateWithTTL() throws ExecutionException, InterruptedException
    {
        ...
        dataStore.query().create().table(keyspace, table).column("graph", Boolean, PartitionKey).column("name", Column.Type.Text)
                .execute();
        dataStore.query().update(keyspace, table).ttl(2).value("name", "Bif")
                .where("graph", Eq, true)
                .execute();
    }
    ...
    public void testCqlExecutionInfo() throws ExecutionException, InterruptedException
    {
        ...
        ResultSet resultSet = dataStore.query().select().star().from(ks.name(), table).where("x", Eq, 1).execute();
    ...
    {
        createKeyspace();
        ResultSet createTable = dataStore.query().create()
                .table(keyspace, table)
                .column("graph", Boolean, PartitionKey)
                .execute();
    ...
    {
        ResultSet result = dataStore.query().create().keyspace(keyspace)
                .ifNotExists()
        ...
        assertThat(keyspace.replication()).isEqualTo(
                ImmutableMap.of("class", "org.apache.cassandra.locator.SimpleStrategy", "replication_factor", "1"));

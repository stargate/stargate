...
public class Starter {
    ...
    private static final String JAR_DIRECTORY = System.getProperty("stargate.libdir", "../stargate-lib");
    ...
    @Pattern(pattern = "^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$", description = "Must be a valid IP address")
    ...
    //Visible for testing
    public Starter(String clusterName, String version, String listenHostStr, String seedHost, Integer seedPort, String dc, String rack, boolean dse, boolean isSimpleSnitch, int cqlPort) {
    ...
    void setStargateProperties()
    {
        ...
        System.setProperty("stargate.snitch_classname", simpleSnitch ? "SimpleSnitch" : "StargateConfigSnitch");
    ...
    public void start() throws BundleException
    {
        ...
        if (files != null)
        {
            ...
            for (File file : files)
            {
                ...
                if (!dse && (name.contains("persistence-dse") || name.contains("persistence-cassandra") && !name.contains("persistence-cassandra-" + version)))
                    continue;

                if (dse && (name.contains("persistence-cassandra") || (name.contains("persistence-dse") && !name.contains("persistence-dse-" + version))))
                    continue;
        ...
        if (!foundVersion)
            throw new IllegalArgumentException(String.format("No persistence backend found for %s %s", (dse ? "dse" : "cassandra"), version));
    ...
    public Optional<Object> getService(String className) throws InvalidSyntaxException
    {
        ...
        {
            ServiceReference<Object> ref = (ServiceReference) b.getBundleContext().getServiceReference(className);
            if (ref != null)
            {
    ...
    private static void printHelp(SingleCommand c) {
        try
        {
            new CliCommandUsageGenerator(79, (a, b) -> {
            ...
            },false ).usage(c.getCommandMetadata(), c.getParserConfiguration(), System.err);
        }
        catch (IOException e)
        {
            throw new IOError(e);

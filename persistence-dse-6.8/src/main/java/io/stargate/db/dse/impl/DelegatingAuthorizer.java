...
public class DelegatingAuthorizer extends CassandraAuthorizer {
  ...
  private static AuthorizedResource resource(IResource resource) {
    ...
    if (resource instanceof FunctionResource) {
      ...
      authResource = ImmutableAuthorizedResource.of(ResourceKind.TABLE);
      ...
      if (dr.isKeyspaceLevel()) {
        throw new UnsupportedOperationException(
            "Keyspace-level data resources are not supported: " + resourcePath);
        ...
        authResource = authResource.withKeyspace(keyspace).withElement(element);
      }
    } else {

...
public class InternalResultSet implements ResultSet
{
                      ...
                      boolean waitedForSchemaAgreement, ExecutionInfo executionInfo)
    {
        ...
        try
        {
            ...
            if (table == null)
            {
                this.table = executor.schema().keyspace(columnSpecification.ksName)
    ...
    private Flowable<? extends org.apache.cassandra.cql3.ResultSet> fetchNext(org.apache.cassandra.cql3.ResultSet result)
    {
        ...
        PagingState nextPagingState = currentPagingOptions == null ? null : null;
        //FIXME: (Jake) not sure the logic here
        if (nextPagingState == null)
            return Flowable.empty();

        return Single.fromFuture(executor.withPagingState(nextPagingState).query()).map(r -> ((ResultMessage.Rows) r).result).toFlowable();
    }
    ...
    private RowIterator<Row> currentIterator()
    {
        if (null == currentIt)
        {
            currentIt = new RowIterator<>(results.count().blockingGet().intValue(),
    ...
    public ByteBuffer getPagingState() {
        return this.executor.getPagingState();
    }

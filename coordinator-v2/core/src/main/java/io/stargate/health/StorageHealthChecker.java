...
@Readiness
@ApplicationScoped
public class StorageHealthChecker implements HealthCheck {
  ...
  public HealthCheckResponse call() {
    if (!STORAGE_CHECK_ENABLED) {
      return HealthCheckResponse.named(NAME)
          .up()
          .withData("status", "Storage check disabled")
          .build();
    }
    try {
      ...
      if (isGreaterThanOrEqual(timestampRead, writeTimestamp)) {
        return HealthCheckResponse.named(NAME)
            .up()
            .withData("status", "Storage is operational")
            .build();
      } else {
        return HealthCheckResponse.named(NAME)
            .down()
            .withData("status", "Storage did not return the proper data.")
            .build();
      }
    } catch (Exception e) {
      ...
      return HealthCheckResponse.named(NAME)
          .down()
          .withData("status", "Unable to access Storage: " + e)
          .build();
    }
  }

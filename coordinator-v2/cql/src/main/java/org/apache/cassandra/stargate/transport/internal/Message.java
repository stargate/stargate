...
import org.apache.cassandra.stargate.transport.internal.messages.*;
...
public abstract class Message {
  ...
  public enum Type {
    ...
    @VisibleForTesting
    public Codec<?> unsafeSetCodec(Codec<?> codec)
  ...
  public String debugString() {
    return String.format(
        "(%s:%s:%s)",
        type, streamId, connection == null ? "null" : connection.getVersion().asInt());
  }

  // Cassandra {4.0.10} Added the following method for stargate
  ...
  public abstract static class Request extends Message {
    ...
    // Cassandra {4.0.10} changed the execute method arguments
    protected abstract CompletableFuture<? extends Response> execute(long queryStartNanoTime);

    // Copied this method as is should not be used
    public final CompletableFuture<? extends Response> execute(
        QueryState queryState, long queryStartNanoTime) {
      ...
      final UUID tracingId = tracingSessionId;
      ...
      if (isTraceable() && isTracingRequested())
        response.thenAccept(resp -> resp.setTracingId(tracingId));
  ...
  public Envelope encode(ProtocolVersion version) {
    ...
    try {
      ...
      if (this instanceof Response) {
        ...
        if (((Request) this).isTracingRequested()) flags.add(Envelope.Header.Flag.TRACING);
      ...
      // version, respond with a protocol error message with the correct message header for that
      // version
      ProtocolVersion responseVersion =
          forcedProtocolVersion == null ? version : forcedProtocolVersion;

      if (responseVersion.isBeta()) flags.add(Envelope.Header.Flag.USE_BETA);
    ...
    private static class RequestDecoder extends Decoder<Request> {
      Request decode(Channel channel, Envelope request) {
        if (request.header.type.direction != Direction.REQUEST)
          throw new ProtocolException(
              String.format(
                  "Unexpected RESPONSE message %s, expecting REQUEST", request.header.type));
    ...
    private static class ResponseDecoder extends Decoder<Response> {
      Response decode(Channel channel, Envelope response) {
        if (response.header.type.direction != Direction.RESPONSE)
          throw new ProtocolException(
              String.format(
                  "Unexpected REQUEST message %s, expecting RESPONSE", response.header.type));

...
(docker-compose logs -f backend-2 &) | grep -q "Starting listening for CQL clients on"

# Bring up the 3rd C* node

docker-compose up -d backend-3
(docker-compose logs -f backend-3 &) | grep -q "Starting listening for CQL clients on"

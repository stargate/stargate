    ...
    public static List<Where<?>> parseWhere(String whereParam, Table tableData) throws IOException {
        ...
        while (fields.hasNext()) {
            ...
            if (!fieldConditions.isObject()) {
                throw new RuntimeException(String.format("Entry for field %s was expecting a JSON object as input.", fieldName));
            }
            ...
            while (ops.hasNext()) {
                ...
                if (!value.isValueNode() || value.isNull()) {
                    throw new RuntimeException(String.format("Value entry for field %s, operation %s was expecting a value, but found an object, array, or null.", fieldName, op));
                }

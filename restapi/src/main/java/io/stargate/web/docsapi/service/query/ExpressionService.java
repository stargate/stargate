...
public class ExpressionService {
     ...
     * <p>
     * This method will {@link #parse(List, JsonNode)} the filter query first and then:
     * <ol>
     *     <li>1. Combine all expression in with And operation</li>
     *     <li>2. Apply {@link RuleSet#simplify(Expression)}</li> operation
     * </ol>
     *
    ...
    public Expression<FilterExpression> constructFilterExpression(List<PathSegment> prependedPath, JsonNode filterJson) {
        ...
        if (parse.isEmpty()) {
            // TODO should be actually throw exception here
     ...
     */
    public List<Expression<FilterExpression>> parse(List<PathSegment> prependedPath, JsonNode filterJson) {
        ...
        while (fields.hasNext()) {
            ...
            Collection<BaseCondition> fieldConditions = conditionProvider.getConditions(field.getValue(), numericBooleans);
            for (BaseCondition fieldCondition : fieldConditions) {
                ImmutableFilterExpression expression = ImmutableFilterExpression.of(filterPath, fieldCondition);
    ...
    /**
     * Resolves the collection/document prepended path and the filed path as specified in the filter query.
     *
     * @param prependedPath Given collection or document path segments
     * @param fieldPath     filed path given in the filter query, expects dot notation, f.e. <code>car.name</code>
     * @return FilterPath
     */
    private FilterPath getFilterPath(List<PathSegment> prependedPath, String fieldPath) {
        ...
        List<String> convertedFieldNamePath = Arrays.stream(fieldNamePath)
                .map(this::convertArrayPath)
                .collect(Collectors.toList());

        if (!prependedPath.isEmpty()) {
            List<String> prependedConverted = prependedPath.stream()
                    .map(pathSeg -> {
        ...
        List<String> path = convertedFieldNamePath.subList(0, convertedFieldNamePath.size() - 1);
        String field = convertedFieldNamePath.get(convertedFieldNamePath.size() - 1);

...
public class DocumentService {
  ...
  public JsonNode getFullDocuments(
      ...
      throws UnauthorizedException {
    final ObjectNode docsResult = mapper.createObjectNode();
  ...
  private JsonNode searchUsingCassandraFilters(
    ...
    if (candidates.size() > paginator.docPageSize) {
      ...
      //      String lastSeenId = existsByDoc.stream().limit(limit).reduce((x,y) -> y).orElse(null);
   ...
   * @param paginator Controls paging for both Docs API and C*
   * @return

...
public class RowsResource {
    ...
    @Timed
    @GET
    public Response getWithWhere(@HeaderParam("X-Cassandra-Token") String token, @PathParam("keyspaceName") final String keyspaceName,
                        @PathParam("tableName") final String tableName, @QueryParam("where") final String where,
                        @QueryParam("pretty") final boolean pretty, @QueryParam("fields") final String fields,
                        @QueryParam("page-size") final int pageSizeParam, @QueryParam("page-state") final String pageStateParam,
                        @QueryParam("raw") final boolean raw, @QueryParam("sort") final String sort) {
        return RequestHandler.handle(() -> {
            ...
            Object response = getRows(fields, raw, sort, localDB, tableMetadata, WhereParser.parseWhere(where, tableMetadata));
            return Response
    ...
    @GET
    @Path("/{path: .*}")
    public Response get(@HeaderParam("X-Cassandra-Token") String token, @PathParam("keyspaceName") final String keyspaceName,
                        @PathParam("tableName") final String tableName, @PathParam("path") List<PathSegment> path,
                        @QueryParam("pretty") final boolean pretty, @QueryParam("fields") final String fields,
                        @QueryParam("page-size") final int pageSizeParam, @QueryParam("page-state") final String pageStateParam,
                        @QueryParam("raw") final boolean raw, @QueryParam("sort") final String sort)
    {
        return RequestHandler.handle(() -> {
            ...
            try {
                ...
                return Response.status(Response.Status.BAD_REQUEST)
                        .entity(new Error("not enough partition keys provided", Response.Status.BAD_REQUEST.getStatusCode()))
                        .build();
            }
    ...
    @Timed
    @POST
    public Response add(@HeaderParam("X-Cassandra-Token") String token, @PathParam("keyspaceName") final String keyspaceName,
                        ...
                        String payload)
    {
        return RequestHandler.handle(() -> {
            ...
            List<Value<?>> values = requestBody.entrySet()
                    .stream()
                    .map((e) -> Converters.colToValue(e, table))
                    .collect(Collectors.toList());

            localDB.query().insertInto(keyspaceName, tableName)
                    .value(values)
    ...
    public Response update(@HeaderParam("X-Cassandra-Token") String token, @PathParam("keyspaceName") final String keyspaceName,
                           @PathParam("tableName") final String tableName, @PathParam("path") List<PathSegment> path,
                           @QueryParam("pretty") final boolean pretty, @QueryParam("raw") final boolean raw, String payload)
    {
        return RequestHandler.handle(() -> modifyRow(token, keyspaceName, tableName, path, pretty, raw, payload));
    }
                           ...
                           @PathParam("tableName") final String tableName, @PathParam("path") List<PathSegment> path)
    {
        return RequestHandler.handle(() -> {
            ...
            localDB.query().delete().from(keyspaceName, tableName).where(where)
    ...
    @PATCH
    @Path("/{path: .*}")
    public Response patch(@HeaderParam("X-Cassandra-Token") String token, @PathParam("keyspaceName") final String keyspaceName,
                           @PathParam("tableName") final String tableName, @PathParam("path") List<PathSegment> path,
                          @QueryParam("pretty") final boolean pretty, @QueryParam("raw") final boolean raw, String payload)
    {
        return RequestHandler.handle(() -> modifyRow(token, keyspaceName, tableName, path, pretty, raw, payload));
    ...
    private Response modifyRow(String token, String keyspaceName, String tableName, List<PathSegment> path, boolean pretty,
                               boolean raw, String payload) throws UnauthorizedException, com.fasterxml.jackson.core.JsonProcessingException, ExecutionException, InterruptedException {
        DataStore localDB = db.getDataStoreForToken(token);
        ...
        Map<String, String> requestBody = mapper.readValue(payload, Map.class);
        List<Value<?>> changes = requestBody.entrySet()
                .stream()
                .map((e) -> Converters.colToValue(e, tableMetadata))
                .collect(Collectors.toList());

        final ResultSet r = localDB.query().update(keyspaceName, tableName).value(changes).where(where)
                .consistencyLevel(ConsistencyLevel.LOCAL_QUORUM).execute();
    ...
    private Object getRows(String fields, boolean raw, String sort, DataStore localDB, Table tableMetadata,
                           List<Where<?>> where) throws Exception {
        List<Column> columns;
        if (Strings.isNullOrEmpty(fields)) {
            columns = tableMetadata.columns();
        } else {
            columns = Arrays.stream(fields.split(",")).map(Column::reference).collect(Collectors.toList());
        }

        final ResultSet r = localDB.query()
                .select()
                .column(columns)
        ...
        List<Map<String, Object>> rows = r.rows().stream().map(Converters::row2Map).collect(Collectors.toList());
    ...
    private List<ColumnOrder> buildSortOrder(String sort) throws com.fasterxml.jackson.core.JsonProcessingException {
        ...
        for (Map.Entry<String, String> entry : sortOrder.entrySet()) {
            Column.Order colOrder = "asc".equals(entry.getValue().toLowerCase()) ? Column.Order.Asc : Column.Order.Desc;
    ...
    private List<Where<?>> buildWhereForPath(Table tableMetadata, List<PathSegment> path) {
        ...
        return IntStream.range(0, path.size())
                .mapToObj(i -> Converters.idToWhere(path.get(i).getPath(), keys.get(i).name(), tableMetadata))
                .collect(Collectors.toList());
    }
}

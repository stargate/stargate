...
public class ColumnResource {
  ...
  public Response getAll(@HeaderParam("X-Cassandra-Token") String token, @PathParam("keyspaceName") final String keyspaceName,
    ...
    return RequestHandler.handle(() -> {
      ...
      List<ColumnDefinition> collect = tableMetadata.columns().stream()
              .map((col) -> new ColumnDefinition(col.name(), col.type().name(), col.kind() == Kind.Static))
  ...
  public Response addColumn(@HeaderParam("X-Cassandra-Token") String token, @PathParam("keyspaceName") final String keyspaceName,
                            @PathParam("tableName") final String tableName, @NotNull final ColumnDefinition columnDefinition) {
    return RequestHandler.handle(() -> {
      DataStore localDB = db.getDataStoreForToken(token);
  ...
  public Response getOne(@HeaderParam("X-Cassandra-Token") String token, @PathParam("keyspaceName") final String keyspaceName,
    ...
    return RequestHandler.handle(() -> {
      ...
      if (col == null) {
        return Response.status(Response.Status.NOT_FOUND).entity(new Error(String.format("column '%s' not found in table", columnName))).build();
      }

      return Response
              .status(Response.Status.OK)
              .entity(new ColumnDefinition(col.name(), col.type().name(), col.kind() == Kind.Static))
              .build();
    });
  }
  ...
  public Response update(@HeaderParam("X-Cassandra-Token") String token, @PathParam("keyspaceName") final String keyspaceName,
    ...
    return RequestHandler.handle(() -> {
      ...
      String alterInstructions = "RENAME " + Converters.maybeQuote(columnName) + " TO " + Converters.maybeQuote(columnUpdate.getNewName());

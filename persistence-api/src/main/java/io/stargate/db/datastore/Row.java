...
public interface Row {
  ...
  default Column column(String column) {
    ...
    Preconditions.checkArgument(
        c != null, "Unknown column '%s' on table '%s'", column, table().name());
    return c;
  }
  ...
  default void verifyColumn(Column column) {
    Preconditions.checkArgument(
        has(column), "Column '%s' is not defined in the Row's metadata.", column.name());
  }

  /**
   * @param column The name of the column
   * @return Figures out the type of the column by inspecting the metadata and returns it.
   * @throws RuntimeException if the column required is not defined in the metadata of the Row.
   *     Check if the Row contains the column first with {@link #has(String)}.
   */
  ...
  /**
   * @throws RuntimeException if the column required is not defined in the metadata of the Row.
   *     Check if the Row contains the column first with {@link #has(Column)}.
   */
  default Object getValue(Column column) {
    try {
      ...
      // then
      // org.apache.cassandra.cql3.UntypedResultSet.Row.has will still throw an
      // IllegalArgumentException. This means

...
public abstract class Column implements SchemaEntity, Comparable<Column>
{
    ...
    {
        AttachmentPoint CUSTOM_ATTACHMENT_POINT = new AttachmentPoint()
        {
             ...
             * DataStoreUtil.getTypeFromDriver(dataType).codec() simply because we use internally slightly different
             * ColumnType <-> Codec mappings than the Driver actually does. One example is that
             * CODEC_REGISTRY.codecFor(DATE) will return DateCodec, but on the server (ColumnUtils) we actually want to
             * use LocalDateCodec for a DATE
             */
            @Override
        ...
        /**
         * Will validate the given value against the underlying column type using <code>instanceof</code>. If the value
         * is a {@link Number} and an <code>instanceof</code> fails, it will also try to check if the value can be
         * narrowed/widened to the given column type using the rules described in
         * {@link NumberCoercion#coerceToColumnType(Class, Object)}.
         *
        ...
        default String toString(Object value)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    throw new IllegalArgumentException(String.format("Codec '%s' cannot process value of type '%s'",
                            codec().getClass().getSimpleName(), value.getClass().getName()), e);
                }
                else
                {
    ...
    public enum Type implements ColumnType
    {
        ...
        Tuple(49, TupleValue.class, false, "tupleOf(<type>...)", "Fixed length sequence of elements of different types")
                {
                    @Override
                    public ColumnType of(ColumnType... types)
                    {
        ...
        Type(final int id, Class<?> javaType, boolean requiresQuotes, String usage, String description)
        {
            this.id = id;
    ...
    public static class ValidationException extends Exception
    {
        ...
        {
            super("Wanted '" + expectedType.cqlDefinition() + "' but got '" + providedType + "' at location '"
                    + location + "'");
        ...
        {
            super("Wanted '" + expectedType.cqlDefinition() + "' but got '<invalid>' at location '" + location + "'");
        ...
        {
            super("Wanted '" + expectedType.cqlDefinition() + "' but got '" + errorMessage + "' at location '"
                    + location + "'");
            this.providedType = providedType.getSimpleName();
            this.expectedType = expectedType.javaType().getSimpleName();
            this.expectedCqlType = expectedType.cqlDefinition();
    ...
    public static final Map<Class<?>, Type> TYPE_MAPPING = ImmutableMap.<Class<?>, Type>builder()
            .put(Date.class, Type.Date)
            .putAll(Arrays.stream(Type.values())
                    .filter(r -> !r.isCollection() && r != Counter && r != Ascii && r != Timeuuid && r != Varchar)
    ...
    public interface Builder
    {
        ...
        default ImmutableColumn.Builder type(@Nullable Class<?> type)
        {
            ...
            Preconditions.checkArgument(cqlType != null,
                    "Tried to set the type for '%s', but this is not a valid CQL type", type.getSimpleName());
            return type(cqlType);
        }

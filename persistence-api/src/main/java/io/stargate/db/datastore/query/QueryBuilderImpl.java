...
import io.stargate.db.datastore.schema.SchemaEntity;
...
@GenerateEmbeddedDSL(modifier = AFModifier.DEFAULT, autoVarArgs = false, name = "QueryBuilder", syntax = "(<keyspace>|<table>|<insert>|<update>|<delete>|<select>|<index>|<type>) consistencyLevel? (future|prepare|execute)", where = {
        ...
        @SubExpr(name = "table", definedAs = "(create table ifNotExists? column+ withComment? (<withVL>|<withEL>)?) | (alter table (((addColumn+)? | (dropColumn+)?) | (withComment? (<withVL>|<withEL>|<withoutVL>|<withoutEL>)?))) | (drop table ifExists?) | (truncate table) | (<renameGraphLabel>)"),
        ...
        @SubExpr(name = "select", definedAs = "select star? column* from (where* limit? orderBy*) allowFiltering? withWriteTimeColumn?"),
        @SubExpr(name = "index", definedAs = "(drop ((materializedView|index) ifExists?)) | (create ((materializedView ifNotExists? asSelect star? (column+) from)"
        ...
        @SubExpr(name = "withEL", definedAs = "withEdgeLabel fromVertexLabel fromColumn+ toVertexLabel toColumn+"),
        @SubExpr(name = "withoutVL", definedAs = "withoutVertexLabel"),
        @SubExpr(name = "withoutEL", definedAs = "withoutEdgeLabel"),
        @SubExpr(name = "renameGraphLabel", definedAs = "alter table rename (vertexLabel|edgeLabel) fromLabel? toLabel"),
})
public class QueryBuilderImpl
{
    ...
    private List<Column> addColumns = new ArrayList<>();
    private List<Column> dropColumns = new ArrayList<>();
    ...
    {
        checkArgument(!isCreate || !isMaterializedView || column.type() == null, "Column '%s' type should not be specified for materialized view '%s'", column.name(),
                index != null ? index.name() : "");
    }

    private void checkTableColumn(Column column)
    {
        checkArgument(!isCreate || !isTable || column.type() != null, "Column '%s' type must be specified for table '%s'", column.name(),
    ...
    {
        value(ImmutableValue.builder().column(column).value(value == null ? Parameter.NULL : value).build());
    }
    ...
    {
        where(ImmutableWhereCondition.builder().column(column).predicate(predicate).value(value).build());
    }
    ...
    public void withWriteTimeColumn(String columnName) { this.writeTimeColumn = columnName; }
    ...
    public <T> MixinPreparedStatement<T> prepare(Class<T> bindingClass)
    {
    ...
    private MixinPreparedStatement createType()
    {
        ...
        query.append(type.columns().stream().map(c -> c.cqlName() + " " + c.type().cqlDefinition())
                .collect(Collectors.joining(", ", " (", ")")));
        return prepareInternal(query.toString());
    }

    protected MixinPreparedStatement<?> prepareInternal(String cql, Optional<Index> index)
    {
        return new MixinPreparedStatement(dataStore.prepare(cql, index), parameters);
    }

    protected MixinPreparedStatement<?> prepareInternal(String cql)
    {
    ...
    private void checkNoType()
    {
        ...
        {
            checkKeyspace();
            checkArgument(schema.keyspace(keyspace.name()).userDefinedType(type.name()) == null,
                    "User defined type '%s.%s' already exists", keyspace.name(), type.name());
        }
    }
    ...
    private void checkType()
    {
        checkKeyspace();
        checkArgument(schema.keyspace(keyspace.name()).userDefinedType(type.name()) != null,
                "Unknown user defined type '%s.%s'", keyspace.name(), type.name());
    }
    ...
    private MixinPreparedStatement deleteQuery()
    {
        ...
        query.append(" WHERE ");
        query.append(getConditions().stream().map(w -> w.column().cqlName() + " " + w.predicate() + " ?")
    ...
    protected MixinPreparedStatement selectQuery()
    {
        checkKeyspace();
    ...
    private MixinPreparedStatement createSelectQuery(Table table, Optional<Index> selectedIndex)
    {
        AbstractTable queryable = selectedIndex.map(i -> i instanceof MaterializedView ? ((AbstractTable) i) : table)
                .orElse(table);
        query.append("SELECT ");
        ...
        {
            query.append(" WHERE ");
            query.append(getConditions().stream().map(this::whereCondition)
                    .collect(Collectors.joining(" AND ")));
        }
        ...
        boolean allowFilteringWithoutIndex = allowFiltering && !wheres.isEmpty() && !selectedIndex.isPresent();
     ...
     * <p>
     * Preserve where conditions that are:
     * <ul>
     * <li>non-search exclusive predicates</li>
     * </ul>
     * Remove where conditions that are:
     * <ul>
     * <li>Or where expressions</li>
     * <li>Any search exclusive predicates</li>
     * </ul>
     * and set the corresponding parameters to ignore
     */
    private List<Where<?>> filterWheresAndRebuildParameters(final List<Where<?>> wheres, List<Parameter<?>> parameters,
                                                            int parameterIndex)
    {
        ...
        for (Where<?> w : wheres)
        {
            if (w instanceof AndWhere)
            {
                ...
                List<Where<?>> andWhereChildrenWheres = filterWheresAndRebuildParameters(andWhere.children(),
                        parameters, parameterIndex);
                if (!andWhereChildrenWheres.isEmpty())
                {
                    preservedWheres.add(
                            AndWhere.and(andWhereChildrenWheres.toArray(new Where<?>[andWhereChildrenWheres.size()])));
                }
            }
    ...
    private List<ColumnOrder> filterOrders(final List<ColumnOrder> orders, Index index)
    {
        ...
        {
            indexedColumnNames = ((AbstractTable) index).clusteringKeyColumns().stream().map(c -> c.name())
                    .collect(toSet());
        }
    ...
    {
        if (w.predicate().equals(WhereCondition.Predicate.EntryEq) && w.value().isPresent() && w.value().get() instanceof Pair)
        {
    ...
    private void checkWheres(AbstractTable queryable)
    {
        ...
        List<String> unknownWhereColNames = getConditions().stream().map(w -> w.column().name())
    ...
    {
        List<String> colNames = queryable.columns().stream().map(c -> c.name()).collect(toList());
        List<String> unknownOrderColNames = orders.stream().map(o -> o.column().name())
                .filter(n -> !colNames.contains(n)).collect(toList());
        checkArgument(unknownOrderColNames.isEmpty(), "Query contains unknown order columns '%s'",
                unknownOrderColNames);

        Map<Column, Long> frequency = orders.stream().collect(Collectors.groupingBy(ColumnOrder::column, counting()));
        frequency.forEach((col, freq) -> checkArgument(freq == 1,
                "Query contained more than one order for column '%s'", col.name()));
    }
    ...
    private Optional<Index> selectIndex(AbstractTable queryable)
    {
        ...
        return indices.stream().sorted(this::indexSort)
                .filter(i -> i.supports(columns, getConditions(), orders, limit)).findFirst();
    }
    ...
    private MixinPreparedStatement createMaterializedView()
    {
        ...
        Set<String> sourceTablePkColumnNames = sourceTable
                .primaryKeyColumns().stream().map(c -> c.name()).collect(toSet());
        Sets.SetView<String> missingColumns = Sets.difference(sourceTablePkColumnNames, columnNames);
        checkArgument(missingColumns.isEmpty(),
                "Materialized view '%s' primary key was missing components %s", index.name(),
                missingColumns);
        List<String> nonPrimaryKeysThatArePartOfMvPrimaryKey = columns.stream()
                .filter(c -> c.kind() != null && c.isPrimaryKeyComponent())
                .map(c -> sourceTable.column(c.name())).filter(c -> !c.isPrimaryKeyComponent())
                .map(c -> c.name())
                .collect(toList());
        checkArgument(nonPrimaryKeysThatArePartOfMvPrimaryKey.size() <= 1,
        ...
        query.append(" WHERE ");
        query.append(columns.stream().filter(c -> c != Column.STAR).map(c -> c.cqlName() + " IS NOT NULL")
                .collect(Collectors.joining(" AND ")));
        query.append(" ");
        key();
        clusteringOrder();
    ...
    private void clusteringOrder()
    {
        if (columns.stream().anyMatch(c -> c.kind() == Column.Kind.Clustering && c.order() != null))
        {
            ...
            query.append(columns.stream().filter(c -> c.kind() == Column.Kind.Clustering)
    ...
    {
        checkArgument(columns.stream().anyMatch(c -> c.kind() == Column.Kind.PartitionKey),
                "At least one partition key must be specified for FROM: '%s' / TO: '%s' vertex labels", fromVertexLabel,
                toVertexLabel);
        query.append("((");
        query.append(columns.stream().filter(c -> c.kind() == Column.Kind.PartitionKey).map(c -> c.cqlName())
                .collect(Collectors.joining(", ")));
        query.append(")");
        ...
        query.append(columns.stream().filter(c -> c.kind() == Column.Kind.Clustering).map(c -> c.cqlName())
                .collect(Collectors.joining(", ")));
        query.append(")");
    }
    ...
    private void checkIndexTypeIfCollection(Column c)
    {
        ...
        if (indexFull)
        {
            checkArgument(column.isFrozenCollection(),
                    "Full indexing can only be used with a frozen list/map/set");
        }
    }
    ...
    private MixinPreparedStatement alterTable()
    {
        ...
        else
        {
            ...
            if (!dropColumns.isEmpty())
            {
                query.append(" DROP (");
                query.append(
                        dropColumns.stream().map(c -> c.cqlName())
                                .collect(Collectors.joining(", ")));
                query.append(")");
            }
            if (comment != null)
    ...
    private MixinPreparedStatement createTable()
    {
        ...
        query.append(
                columns.stream()
                        .map(c -> c.cqlName() + " " + c.type().cqlDefinition() + (c.kind() == Column.Kind.Static ? " STATIC" : ""))
                        .collect(Collectors.joining(", ")));
        query.append(", ");
        key();
        query.append(")");
    ...
    private void key()
    {
        checkArgument(columns.stream().anyMatch(c -> c.kind() == Column.Kind.valueOf(Column.Kind.PartitionKey.name())),
                "At least one partition key must be specified for table or materialized view '%s.%s' %s",
                keyspace.name(),
                table != null ? table.name() : index.name(), Arrays.deepToString(columns.toArray()));
    ...
    private void checkKeyspace()
    {
        if (keyspace != null)
        {
            checkArgument(schema.keyspace(keyspace.name()) != null,
                    "Unknown keyspace '%s'", keyspace.name());
        }
    }
    ...
    private void checkNoKeyspace()
    {
        ...
        {
            checkArgument(schema.keyspace(keyspace.name()) == null,
                    "Keyspace '%s' already exists", keyspace.name());
        }
    }
    ...
    private void checkTable()
    {
        ...
        {
            checkKeyspace();
            checkArgument(schema.keyspace(keyspace.name()).table(table.name()) != null,
                    "Unknown table '%s.%s'", keyspace.name(), table.name());
        }
    }
    ...
    private void checkNoTable()
    {
        if (table != null)
        {
            checkKeyspace();
            checkArgument(schema.keyspace(keyspace.name()).table(table.name()) == null,
                    "Table '%s.%s' already exists", keyspace.name(), table.name());
        }
    }
    ...
    private void checkIndex()
    {
        if (index != null)
        {
            ...
            checkTable();
            checkArgument(
                    schema.keyspace(keyspace.name()).tables().stream().flatMap(t -> t.indexes().stream())
                            .anyMatch(i -> index.name().equals(i.name())),
                    "Unknown index '%s.%s'", keyspace.name(), index.name());
        }
    }
    ...
    private void checkNoIndex()
    {
        if (index != null)
        {
            ...
            checkArgument(
                    ...
                    "Index '%s.%s' already exists", keyspace.name(), index.name());
        }
    }
    ...
    private void checkColumns(AbstractTable queryable)
    {
        if (!columns.isEmpty())
        {
            columns.stream().filter(c -> c != Column.STAR).forEach(c ->
            {
                checkArgument(queryable.column(c.name()) != null, "Unknown column '%s' on '%s.%s'",
                        c.name(),
                        keyspace.name(), queryable.name());
            });
        }
    }

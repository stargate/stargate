...
public abstract class UserDefinedType implements Column.ColumnType, SchemaEntity
{
    ...
    public Object validate(Object value, String location) throws Column.ValidationException
    {
        ...
        if (null != value)
        {
            if (!(value instanceof UdtValue) || !((UdtValue) value).getType().getName().asInternal().equals(name()))
            {
    ...
    public UdtValue create(Object... fields)
    {
        ...
        Preconditions.checkArgument(fields.length == 0 || fields.length == columns().size(),
                "Expected %s parameter(s) when initializing '%s' but got %s", columns().size(), name(), fields.length);
        try
        {
    ...
    public UserDefinedType dereference(Keyspace keyspace)
    {
        ...
        Preconditions.checkArgument(userDefinedType != null,
                "User defined type '%s' does not exist in keyspace '%s'", name(), keyspace.name());
    ...
    public Column.ColumnType fieldType(String name)
    {
        ...
        Preconditions.checkArgument(null != column, "User defined type '%s' does not have field '%s'", cqlName(),
                name);
        return column.type();
    }
}

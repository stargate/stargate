...
class GrpcClientsTest extends BridgeTest {
    ...
    public void happyPath() {
        ...
        doAnswer(invocationOnMock -> {
            ...
            return null;
        }).when(bridgeService).describeKeyspace(any(), any());
        ...
        MutinyStargateBridgeGrpc.MutinyStargateBridgeStub client = grpcClients.bridgeClient(requestInfo);
        UniAssertSubscriber<Schema.CqlKeyspaceDescribe> result = client.describeKeyspace(query).subscribe().withSubscriber(UniAssertSubscriber.create());
        ...
        assertThat(headersCaptor.getAllValues()).singleElement()
                .satisfies(metadata -> {
                    Metadata.Key<String> tenantKey = Metadata.Key.of("custom-tenant-key", Metadata.ASCII_STRING_MARSHALLER);
                    assertThat(metadata.get(tenantKey)).isEqualTo(tenant);
                    Metadata.Key<String> tokenKey = Metadata.Key.of("custom-token-key", Metadata.ASCII_STRING_MARSHALLER);

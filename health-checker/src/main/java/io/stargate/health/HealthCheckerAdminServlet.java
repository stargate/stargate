...
/**
 * Custom version of DropWizard's {@link AdminServlet}, in order to use Prometheus's metrics servlet
 * instead of DropWizard's.
 *
 * <p>Unfortunately the original class is not really designed for extension, so we need to duplicate
 * it entirely.
 */
public class HealthCheckerAdminServlet extends HttpServlet {
  ...
  public void init(ServletConfig config) throws ServletException {
    ...
    this.healthcheckUri =
        getParam(config.getInitParameter(HEALTHCHECK_URI_PARAM_KEY), DEFAULT_HEALTHCHECK_URI);
    this.cpuprofileUri =
        getParam(config.getInitParameter(CPU_PROFILE_URI_PARAM_KEY), DEFAULT_CPU_PROFILE_URI);

...
public interface ByteSource {
  ...
  /**
   * Encodes byte-accessible data as a byte-comparable source that has 0s escaped and finishes in an
   * escaped state. This provides a weakly-prefix-free byte-comparable version of the content to use
   * in sequences. (See {@link AbstractEscaper} for a detailed explanation.)
   */
  ...
  /**
   * Produce a source for a signed fixed-length number, also translating empty to null. The first
   * byte has its sign bit inverted, and the rest are passed unchanged. Presumes that the length of
   * the buffer is always either 0 or constant for the type, which permits decoding and ensures the
   * representation is prefix-free.
   */
  ...
  /**
   * Produce a source for a signed fixed-length number. The first byte has its sign bit inverted,
   * and the rest are passed unchanged. Presumes that the length of the buffer is always constant
   * for the type.
   */
  ...
  /**
   * Produce a source for a signed fixed-length floating-point number, also translating empty to
   * null. If sign bit is on, returns negated bytes. If not, add the sign bit value. (Sign of IEEE
   * floats is the highest bit, the rest can be compared in magnitude by byte comparison.) Presumes
   * that the length of the buffer is always either 0 or constant for the type, which permits
   * decoding and ensures the representation is prefix-free.
   */
  ...
  /**
   * Produce a source for a signed fixed-length floating-point number. If sign bit is on, returns
   * negated bytes. If not, add the sign bit value. (Sign of IEEE floats is the highest bit, the
   * rest can be compared in magnitude by byte comparison.) Presumes that the length of the buffer
   * is always constant for the type.
   */
  ...
  /**
   * Fixed length signed number encoding. Inverts first bit (so that neg < pos), then just posts all
   * bytes from the buffer. Assumes buffer is of correct length.
   */
  static class SignedFixedLengthNumber<V> implements ByteSource {
    ...
    @Override
    public int next() {
      if (bufpos >= accessor.size(data)) return END_OF_STREAM;
  ...
  /**
   * Fixed length signed floating point number encoding. First bit is sign. If positive, add sign
   * bit value to make greater than all negatives. If not, invert all content to make negatives with

...
1. `init()` - it will initialize the internal components of this CDC producer including underlying link:https://docs.confluent.io/current/clients/producer.html[Kafka Producer]. The configuration is taken from `ConfigStore` it is described in <<Configuration>>

...
public class ResultMessage extends Message.Response
{
    ...
    {
        public static final CBCodec<Result.ResultMetadata> METADATA_CODEC = new CBCodec<Result.ResultMetadata>()
        {
            @Override
            public Result.ResultMetadata decode(ByteBuf body, ProtocolVersion version)
            {
                ...
                if (flags.contains(Result.Flag.METADATA_CHANGED))
                {
                    ...
                    assert !flags.contains(Result.Flag.NO_METADATA) : "MetadataChanged and NoMetadata are mutually exclusive flags";
            ...
            @Override
            public void encode(Result.ResultMetadata metadata, ByteBuf dest, ProtocolVersion version)
            {
                ...
                if (hasMorePages)
                    CBUtil.writeValue(metadata.pagingState, dest);
            ...
            public int encodedSize(Result.ResultMetadata metadata, ProtocolVersion version)
            {
                ...
                int size = 8;
                if (hasMorePages)
                    size += CBUtil.sizeOfValue(metadata.pagingState);
    ...
    {
        public static final CBCodec<Result.PreparedMetadata> METADATA_CODEC = new CBCodec<Result.PreparedMetadata>()
        {
            @Override
            ...
            @Override
            public void encode(Result.PreparedMetadata metadata, ByteBuf dest, ProtocolVersion version)
            {
                ...
                if (version.isGreaterOrEqualTo(ProtocolVersion.V4))
                {
                    ...
                    else
                    {
                        ...
                        for (Short bindIndex : metadata.partitionKeyBindIndexes)
            ...
            public int encodedSize(Result.PreparedMetadata metadata, ProtocolVersion version)
            {
                ...
                if (metadata.partitionKeyBindIndexes != null && version.isGreaterOrEqualTo(ProtocolVersion.V4))
    ...
    {
        public static final CBCodec<Result.SchemaChangeMetadata> METADATA_CODEC = new CBCodec<Result.SchemaChangeMetadata>()
        {
            @Override
            ...
            @Override
            public void encode(Result.SchemaChangeMetadata metadata, ByteBuf dest, ProtocolVersion version)
            {
        ...
        public void encode(Result result, ByteBuf dest, ProtocolVersion version)
        {
            ...
            if (version.isGreaterOrEqualTo(ProtocolVersion.V3))
            {
                ...
                CBUtil.writeAsciiString(metadata.target, dest);
                CBUtil.writeAsciiString(metadata.keyspace, dest);
                if (!metadata.target.equals("KEYSPACE"))
                    CBUtil.writeAsciiString(metadata.name, dest);
            }
            else
            {
        ...
        public int encodedSize(Result result, ProtocolVersion version)
        {
            ...
            if (version.isGreaterOrEqualTo(ProtocolVersion.V3))
            {
                int size = CBUtil.sizeOfAsciiString(metadata.change)
                         + CBUtil.sizeOfAsciiString(metadata.target)
                         + CBUtil.sizeOfAsciiString(metadata.keyspace);

                if (!metadata.target.equals("KEYSPACE"))
                    size += CBUtil.sizeOfAsciiString(metadata.name);

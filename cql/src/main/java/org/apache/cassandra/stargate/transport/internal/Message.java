...
public abstract class Message
{
    ...
    public static abstract class Request extends Message
    {
        ...
        protected abstract CompletableFuture<? extends Response> execute(Persistence persistence, QueryState queryState, long queryStartNanoTime);
    ...
    public static class Dispatcher extends SimpleChannelInboundHandler<Request>
    {
        ...
        void processRequest(ChannelHandlerContext ctx, Request request)
        {
            ...
            try
            {
                ...
                CompletableFuture<? extends  Response> req = request.execute(persistence, qstate, queryStartNanoTime);

                req.whenComplete((response, err) ->
                {
                    if (err != null)
                    {
        ...
        private void handleError(ChannelHandlerContext ctx, Message.Request request, Throwable error)
        {
            try
            {
                ...
                if (error instanceof CompletionException)
                    error = error.getCause();
                flush(new Message.Dispatcher.FlushItem(ctx, ErrorMessage.fromException(error, handler).setStreamId(request.getStreamId()), request.getSourceFrame(), this));
            }
            catch (Throwable t)
            {
                request.getSourceFrame().release(); // ok to release since flush was the last call and does not throw after adding the item to the queue

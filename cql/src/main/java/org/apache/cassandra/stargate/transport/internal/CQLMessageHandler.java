...
/**
 * Implementation of {@link AbstractMessageHandler} for processing CQL messages which comprise a
 * {@link Message} wrapped in an {@link Envelope}. This class is parameterized by a {@link Message}
 * subtype, expected to be either {@link Message.Request} or {@link Message.Response}. Most
 * commonly, an instance for handling {@link Message.Request} is created for each inbound CQL client
 * connection.
 *
 * <p># Small vs large messages Small messages are deserialized in place, and then handed off to a
 * consumer for processing. Large messages accumulate frames until all bytes for the envelope are
 * received, then concatenate and deserialize the frames on the event loop thread and pass them on
 * to the same consumer.
 *
 * <p># Flow control (backpressure) The size of an incoming message is explicit in the {@link
 * Envelope.Header}.
 *
 * <p>By default, every connection has 1MiB of exlusive permits available before needing to access
 * the per-endpoint and global reserves. By default, those reserves are sized proportionally to the
 * heap - 2.5% of heap per-endpoint and a 10% for the global reserve.
 *
 * <p>Permits are held while CQL messages are processed and released after the response has been
 * encoded into the buffers of the response frame.
 *
 * <p>A connection level option (THROW_ON_OVERLOAD) allows clients to choose the backpressure
 * strategy when a connection has exceeded the maximum number of allowed permits. The choices are to
 * either pause reads from the incoming socket and allow TCP backpressure to do the work, or to
 * throw an explict exception and rely on the client to back off.
 */
public class CQLMessageHandler<M extends Message> extends AbstractMessageHandler {
  ...
  protected boolean processOneContainedMessage(
      ShareableBytes bytes, Limit endpointReserve, Limit globalReserve) {
    ByteBuffer buf = bytes.get();
    Envelope.Decoder.HeaderExtractionResult extracted = envelopeDecoder.extractHeader(buf);
    if (!extracted.isSuccess())
      return handleProtocolException(
  ...
  private boolean handleProtocolException(
    ...
    if (expectedMessageLength < 0
      ...
      buf.position(
          Math.min(
              buf.limit(),
              buf.position() + Envelope.Header.LENGTH + Ints.checkedCast(expectedMessageLength)));
  ...
  protected boolean processRequest(Envelope request) {
    ...
    try {
      ...
      if (message != null) request.release();
  ...
  /**
   * For "expected" errors this ensures we pass a WrappedException, which contains a streamId, to
   * the error handler. This makes sure that whereever possible, the streamId is propagated back to
   * the client. This also releases the capacity acquired for processing as indicated by supplied
   * header.
   *
   * @param t
   * @param header
   ...
   * For "expected" errors this ensures we pass a WrappedException, which contains a streamId, to
   * the error handler. This makes sure that whereever possible, the streamId is propagated back to
   * the client. This variant doesn't call release as it is intended for use when an error occurs
   * without any capacity being acquired. Typically, this would be the result of an acquisition
   * failure if the THROW_ON_OVERLOAD option has been specified by the client.
   *
   * @param t
   * @param header
   */
  ...
  /**
   * For use in the case where the error can't be mapped to a specific stream id, such as a
   * corrupted frame, or when extracting a CQL message from the frame's payload fails. This does not
   * attempt to release any resources, as these errors should only occur before any capacity
   * acquisition is attempted (e.g. on receipt of a corrupt frame, or failure to extract a CQL
  ...
  protected boolean processFirstFrameOfLargeMessage(
      IntactFrame frame, Limit endpointReserve, Limit globalReserve) throws IOException {
  ...
  private boolean acquireCapacityAndQueueOnFailure(
      Envelope.Header header, Limit endpointReserve, Limit globalReserve) {
    int bytesRequired = Ints.checkedCast(header.bodySizeInBytes);
    long currentTimeNanos = approxTime.now();
    return acquireCapacity(
  ...
  private boolean acquireCapacity(
      Envelope.Header header, Limit endpointReserve, Limit globalReserve) {
    int bytesRequired = Ints.checkedCast(header.bodySizeInBytes);
    return acquireCapacity(endpointReserve, globalReserve, bytesRequired)
  ...
  protected void processCorruptFrame(FrameDecoder.CorruptFrame frame) {
    ...
    // If this is part of a multi-frame message, process it before passing control to the error
    // handler.
  ...
  private class LargeMessage extends AbstractMessageHandler.LargeMessage<Envelope.Header> {
    ...
    private Envelope assembleFrame() {
      ByteBuf body =
          Unpooled.wrappedBuffer(
              buffers.stream().map(ShareableBytes::get).toArray(ByteBuffer[]::new));
    ...
    /**
     * Used to indicate that a message should be dropped and not processed. We do this on receipt of
     * the first frame of a large message if sufficient capacity cannot be acquired to process it
     * and throwOnOverload is set for the connection. In this case, the client has elected to shed
     * load rather than apply backpressure so we must ensure that subsequent frames are consumed
     * from the channel. At that point an error response is returned to the client, rather than
    ...
    protected void onComplete() {
      if (overloaded)
        handleErrorAndRelease(
            new OverloadedException(
                "Server is in overloaded state. " + "Cannot accept more requests at this point"),
            header);
      else if (!isCorrupt) processRequest(assembleFrame());
    }

...
/**
 * Takes care of intializing a Netty Channel and Pipeline for client protocol connections. The
 * pipeline is first set up with some common handlers for connection limiting, dropping idle
 * connections and optionally SSL, along with a handler to deal with the handshake between client
 * and server. That handshake handler calls back to this class to reconfigure the pipeline once the
 * protocol version for the connection has been established.
 */
public class PipelineConfigurator {
  ...
  public ChannelFuture initializeChannel(
    ...
    logger.info(
        ...
        socket,
        tlsEncryptionPolicy.description());
                  ...
                  new ByteToMessageDecoder() {
                    ...
                    protected void decode(
                      ...
                      if (SslHandler.isEncrypted(byteBuf)) {
                        // Connection uses SSL/TLS, replace the detection handler with a SslHandler
                        // and so use
                        // encryption.
                        SslHandler sslHandler = sslContext.newHandler(channel.alloc());
                        channelHandlerContext
                            .pipeline()
                            .replace(SSL_HANDLER, SSL_HANDLER, sslHandler);
                      } else {
                        // Connection use no TLS/SSL encryption, just remove the detection handler
  ...
  protected FrameDecoder frameDecoder(String compression, BufferPoolAllocator allocator) {
    if (null == compression) return FrameDecoderCrc.create(allocator);
    if (compression.equalsIgnoreCase("LZ4")) return FrameDecoderLZ4.fast(allocator);
  ...
  protected FrameEncoder frameEncoder(String compression) {
    if (Strings.isNullOrEmpty(compression)) return FrameEncoderCrc.instance;
    if (compression.equalsIgnoreCase("LZ4")) return FrameEncoderLZ4.fastInstance;
    throw new ProtocolException("Unsupported compression type: " + compression);
  }
  ...
  public void configureLegacyPipeline(
    ...
    pipeline.addBefore(
        INITIAL_HANDLER,
        LEGACY_MESSAGE_PROCESSOR,
        new PreV5Handlers.LegacyDispatchHandler(dispatcher, limits));
    pipeline.remove(INITIAL_HANDLER);
    onNegotiationComplete(pipeline);
  }
}

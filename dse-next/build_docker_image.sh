...
CASSTAG="$(cd ../coordinator; ./mvnw -pl persistence-dse-next help:evaluate -Dexpression=cassandra.version -q -DforceStdout)"
...
echo "Building docker image $REPO/dse-next:$CASSTAG using cassandra from $CASSANDRA_DIR"
...
docker buildx build --build-arg CASSANDRA_DIR=$CASSANDRA_DIR -t $REPO/dse-next:$CASSTAG $DOCKER_FLAGS .

...
import static org.apache.cassandra.stargate.transport.internal.Message.logger;
...
public class PreV5Handlers {
  /**
   * Wraps an {@link org.apache.cassandra.transport.Dispatcher} so that it can be used as an channel
   * inbound handler in pre-V5 pipelines.
   */
  public static class LegacyDispatchHandler extends SimpleChannelInboundHandler<Message.Request> {
    ...
    private Flusher.FlushItem.Unframed toFlushItem(
        Channel channel, Message.Request request, Message.Response response) {
      return new Flusher.FlushItem.Unframed(
          channel, response, request.getSource(), this::releaseItem);
    }

    private void releaseItem(Flusher.FlushItem<Message.Response> item) {
      // Note: in contrast to the equivalent for V5 protocol, CQLMessageHandler::release(FlushItem
      // item),
      // this does not release the FlushItem's Message.Response. In V4, the buffers for the
      // response's body
      // and serialised header are emitted directly down the Netty pipeline from Envelope.Encoder,
      // so
     ...
     * in one of the first handlers in the pipeline (Envelope.Decoder::decode()). However, incase of
     * any exception thrown between that handler (where inflight payload is incremented) and this
     ...
     * the message envelope associated with the exception.
     *
  ...
  /**
   * Simple adaptor to allow {@link
   * org.apache.cassandra.stargate.transport.internal.Message.Decoder#decodeMessage(Channel,
   * Envelope)} to be used as a handler in pre-V5 pipelines
   */
  ...
  /**
   * Pre-V5 exception handler which closes the connection if an {@link
   * org.apache.cassandra.transport.ProtocolException} is thrown
   */
  @ChannelHandler.Sharable
  public static final class ExceptionHandler extends ChannelInboundHandlerAdapter {
    ...
    public void exceptionCaught(final ChannelHandlerContext ctx, Throwable cause) {
      ...
      if (ctx.channel().isOpen()) {
        ...
        // Most cases of PE are wrapped so the type check below is expected to fail more often than
        // not.
        // At this moment Fatal exceptions are not thrown in v4, but just as a precaustion we check
        // for them here
    ...
    private static boolean isFatal(Throwable cause) {
      return cause
          instanceof
          ProtocolException; // this matches previous versions which didn't annotate exceptions as
      // fatal or not
    }
  }

...
public class Envelope {
   ...
   *
   * <p>The header is defined the following way in native protocol version 3 and later:
   *
   * <p>0 8 16 24 32 40 +---------+---------+---------+---------+---------+ | version | flags |
  ...
  public static Envelope create(
      ...
      EnumSet<Header.Flag> flags,
      ByteBuf body) {
    Header header = new Header(version, flags, streamId, type, (long) body.readableBytes());
  ...
  public ByteBuf encodeHeader() {
    ...
    // Continue to support writing pre-v3 headers so that we can give proper error messages to
    // drivers that
    // connect with the v1/v2 protocol. See CASSANDRA-11464.
    if (header.version.isGreaterOrEqualTo(ProtocolVersion.V3)) buf.writeShort(header.streamId);
    else buf.writeByte(header.streamId);
  ...
  public static class Decoder extends ByteToMessageDecoder {
     ...
     * without modifying the position of the underlying buffer. This essentially mirrors the pre-V5
     * code in {@link Decoder#decode(ByteBuf)}, with three differences:
     *
     * <ul>
     *   <li>The input is a ByteBuffer rather than a ByteBuf
     *   <li>This cannot return null, as V5 always deals with entire CQL messages. Coalescing of
     *       bytes off the wire happens at the layer below, in {@link
     *       org.apache.cassandra.net.FrameDecoder}
     *   <li>This method never throws {@link ProtocolException}. Instead, a subclass of {@link
     *       HeaderExtractionResult} is returned which may provide either a {@link Header} or a
     *       {@link ProtocolException},depending on the result of its {@link
     *       HeaderExtractionResult#isSuccess()} method.
     * </ul>
     *
    ...
    HeaderExtractionResult extractHeader(ByteBuffer buffer) {
      Preconditions.checkArgument(
          buffer.remaining() >= Header.LENGTH,
      ...
      // if a negative length is read, return error but report length as 0 so we don't attempt to
      // skip
      if (bodyLength < 0)
        return new HeaderExtractionResult.Error(
      ...
      try {
        ...
        return new HeaderExtractionResult.Success(
            new Header(version, decodedFlags, streamId, type, bodyLength));
    ...
    public abstract static class HeaderExtractionResult {
      ...
      ProtocolException error() {
        throw new IllegalStateException(
            String.format("Unable to provide error from extraction result : %s", outcome));
      }
    ...
    Envelope decode(ByteBuf buffer) {
      ...
      // Check the first byte for the protocol version before we wait for a complete header.
      // Protocol versions

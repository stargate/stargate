...
import static org.apache.cassandra.stargate.transport.internal.CQLMessageHandler.envelopeSize;
...
abstract class Flusher implements Runnable {
  ...
  public static final int MAX_FRAMED_PAYLOAD_SIZE =
      Math.min(
          BufferPool.NORMAL_CHUNK_SIZE,
          FrameEncoder.Payload.MAX_SIZE
              - Math.max(
                  FrameEncoderCrc.HEADER_AND_TRAILER_LENGTH,
  ...
  static class FlushItem<T> {
    ...
    FlushItem(
        Kind kind, Channel channel, T response, Envelope request, Consumer<FlushItem<T>> tidy) {
      this.kind = kind;
  ...
  private void processFramedResponse(FlushItem.Framed flush) {
    ...
    if (envelopeSize(outbound.header) >= MAX_FRAMED_PAYLOAD_SIZE) {
      ...
      payloads
          .computeIfAbsent(flush.channel, channel -> new FlushBuffer(channel, flush.allocator, 5))
          .add(flush.response);
    }
  }

  private void flushLargeMessage(
      Channel channel, Envelope outbound, FrameEncoder.PayloadAllocator allocator) {
    ...
    while (body.readableBytes() > 0 || firstFrame) {
      ...
      if (payloadSize >= MAX_FRAMED_PAYLOAD_SIZE) buf.limit(MAX_FRAMED_PAYLOAD_SIZE);
      ...
      if (remaining > 0) buf.put(body.slice(body.readerIndex(), remaining).nioBuffer());
  ...
  private void writeAndFlush(Channel channel, FrameEncoder.Payload payload) {
    // we finish, but not "release" here since we're passing the buffer ownership to
    // FrameEncoder#encode
    payload.finish();
  ...
  protected boolean processQueue() {
    ...
    while ((flush = poll()) != null) {
      if (flush.kind == FlushItem.Kind.FRAMED) processFramedResponse((FlushItem.Framed) flush);
      else processUnframedResponse((FlushItem.Unframed) flush);
  ...
  private static final class LegacyFlusher extends Flusher {
    ...
    public void run() {
      ...
      if (doneWork) {
        ...
        if (++runsWithNoWork > 5) {
          scheduled.set(false);
          if (isEmpty() || !scheduled.compareAndSet(false, true)) return;
        }
      }

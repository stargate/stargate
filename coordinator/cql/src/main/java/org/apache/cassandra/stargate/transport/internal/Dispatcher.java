...
public class Dispatcher {
   ...
   */
  static CompletableFuture<Message.Response> processRequest(
      ServerConnection connection, Message.Request request) {
    long queryStartNanoTime = System.nanoTime();
    if (connection.getVersion().isGreaterOrEqualTo(ProtocolVersion.V4))
      ClientWarn.instance.captureWarnings();
    // Cassandra {4.0.10} patched to not get queryState
  ...
  void processRequest(Channel channel, Message.Request request, FlushItemConverter forFlusher) {
    ...
    try {
      ...
      // Cassandra {4.0.10} Patched to support async responses
  ...
  public static void shutdown() {
    // Cassandra {4.0.10} Patched to not shutdown requestExecutor

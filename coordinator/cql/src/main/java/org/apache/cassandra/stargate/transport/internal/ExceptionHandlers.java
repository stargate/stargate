...
public class ExceptionHandlers {
  ...
  public static ChannelInboundHandlerAdapter postV5Handler(
      FrameEncoder.PayloadAllocator allocator, ProtocolVersion version) {
  ...
  private static final class PostV5ExceptionHandler extends ChannelInboundHandlerAdapter {
    ...
    public void exceptionCaught(final ChannelHandlerContext ctx, Throwable cause) {
      ...
      if (ctx.channel().isOpen()) {
        ...
        FrameEncoder.Payload payload =
            allocator.allocate(true, CQLMessageHandler.envelopeSize(response.header));
        try {
          ...
          if (isFatal(cause)) promise.addListener(future -> ctx.close());
    ...
    private static boolean isFatal(Throwable cause) {
      return Throwables.anyCauseMatches(
          cause, t -> t instanceof ProtocolException && ((ProtocolException) t).isFatal());
    }
  }

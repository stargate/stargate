...
/**
 * {@link ChannelInboundHandlerAdapter} implementation which allows to limit the number of
 * concurrent connections to the Server. Be aware this <strong>MUST</strong> be shared between all
 * child channels.
 */
...
final class ConnectionLimitHandler extends ChannelInboundHandlerAdapter {
  ...
  private static final AttributeKey<InetAddress> addressAttributeKey =
      AttributeKey.valueOf("org.apache.cassandra.transport.ConnectionLimitHandler#address");
  ...
  private static InetAddress setRemoteAddressAttribute(Channel channel) {
    ...
    if (remoteAddress instanceof InetSocketAddress) {
      ...
      noSpamLogger.warn(
          "Remote address of unknown type: {}, skipping per-IP connection limits",
  ...
  public void channelActive(ChannelHandlerContext ctx) throws Exception {
    ...
    if (count > limit) {
      ...
      noSpamLogger.error(
          "Exceeded maximum native connection limit of {} by using {} connections (see native_transport_max_concurrent_connections in cassandra.yaml)",
          limit,
          count);
      ctx.close();
    } else {
      ...
      if (perIpLimit > 0) {
        ...
        if (perIpCount.incrementAndGet() > perIpLimit) {
          // The decrement will be done in channelClosed(...)
          noSpamLogger.error(
              "Exceeded maximum native connection limit per ip of {} by using {} connections (see native_transport_max_concurrent_connections_per_ip)",
              perIpLimit,
              perIpCount);
          ctx.close();
          return;

...
public class DataTypeTest extends BaseIntegrationTest {
  ...
  private static List<TypeSample<?>> generateAllTypes(CqlIdentifier keyspaceId) {
    List<TypeSample<?>> primitiveTypes =
        Arrays.asList(
            TypeSample.typeSample(DataTypes.ASCII, GenericType.STRING, "sample ascii"),
            TypeSample.typeSample(DataTypes.BIGINT, GenericType.LONG, Long.MAX_VALUE),
            TypeSample.typeSample(
                DataTypes.BLOB, GenericType.of(ByteBuffer.class), Bytes.fromHexString("0xCAFE")),
            TypeSample.typeSample(DataTypes.BOOLEAN, GenericType.BOOLEAN, Boolean.TRUE),
            TypeSample.typeSample(
                DataTypes.DECIMAL, GenericType.BIG_DECIMAL, new BigDecimal("12.3E+7")),
            TypeSample.typeSample(DataTypes.DOUBLE, GenericType.DOUBLE, Double.MAX_VALUE),
            TypeSample.typeSample(DataTypes.FLOAT, GenericType.FLOAT, Float.MAX_VALUE),
            TypeSample.typeSample(DataTypes.INET, GenericType.INET_ADDRESS, sampleInet()),
            TypeSample.typeSample(DataTypes.TINYINT, GenericType.BYTE, Byte.MAX_VALUE),
            TypeSample.typeSample(DataTypes.SMALLINT, GenericType.SHORT, Short.MAX_VALUE),
            TypeSample.typeSample(DataTypes.INT, GenericType.INTEGER, Integer.MAX_VALUE),
            TypeSample.typeSample(
                DataTypes.DURATION, GenericType.CQL_DURATION, CqlDuration.from("PT30H20M")),
            TypeSample.typeSample(DataTypes.TEXT, GenericType.STRING, "sample text"),
            TypeSample.typeSample(
                DataTypes.TIMESTAMP, GenericType.INSTANT, Instant.ofEpochMilli(872835240000L)),
            TypeSample.typeSample(
                DataTypes.DATE, GenericType.LOCAL_DATE, LocalDate.ofEpochDay(16071)),
            TypeSample.typeSample(
    ...
    for (TypeSample<?> type : primitiveTypes) {
      ...
      allTypes.add(type);
      allTypes.add(TypeSample.listOf(type));
      if (!type.javaType.equals(GenericType.CQL_DURATION)) {
        allTypes.add(TypeSample.setOf(type));
        allTypes.add(TypeSample.mapToIntFrom(type));
      }
      allTypes.add(TypeSample.mapOfIntTo(type));
      allTypes.add(TypeSample.tupleOfIntAnd(type));
      allTypes.add(TypeSample.udtOfIntAnd(type, keyspaceId));
    }
    return allTypes;
  }

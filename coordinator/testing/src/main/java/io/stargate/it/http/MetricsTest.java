...
public class MetricsTest extends BaseIntegrationTest {
  ...
  public void dropwizardMetricReadSizeIsPresent() throws IOException {
    ...
    assumeThat(backend.isDse()).as("Disabled because this metric does not exists for Cassandra 4 backend").isTrue();

    String expectedMetric = "persistence_dse_" + StringUtils.remove(backend.clusterVersion(), '.').substring(0, 2) + "_org_apache_cassandra_metrics_client_request_read_size_histogram{";
    String result = RestUtils.get("", String.format("%s:8084/metrics", host), HttpStatus.SC_OK);
    List<String> lines =
        Arrays.stream(result.split(System.getProperty("line.separator")))

 ...
 */
package io.stargate.bridge.retries;
...
class DefaultRetryPolicyTest {
  ...
  class OnReadTimeout {
    ...
    public void shouldProcessReadTimeouts() {
      assertOnReadTimeout(QUORUM, 2, 2, false, 0).isEqualTo(RetryDecision.RETRY);
      assertOnReadTimeout(QUORUM, 2, 2, false, 1).isEqualTo(RetryDecision.RETHROW);
      assertOnReadTimeout(QUORUM, 2, 2, true, 0).isEqualTo(RetryDecision.RETHROW);
      assertOnReadTimeout(QUORUM, 2, 1, true, 0).isEqualTo(RetryDecision.RETHROW);
      assertOnReadTimeout(QUORUM, 2, 1, false, 0).isEqualTo(RetryDecision.RETHROW);
    }
  ...
  class OnWriteTimeout {
    ...
    public void shouldProcessWriteTimeouts() {
      assertOnWriteTimeout(QUORUM, BATCH_LOG, 2, 0, 0).isEqualTo(RetryDecision.RETRY);
      assertOnWriteTimeout(QUORUM, BATCH_LOG, 2, 0, 1).isEqualTo(RetryDecision.RETHROW);
      assertOnWriteTimeout(QUORUM, SIMPLE, 2, 0, 0).isEqualTo(RetryDecision.RETHROW);
    }
  ...
  class onUnprepared {
    ...
    public void retry() {
      ...
      assertThat(decisionFirst).isEqualTo(decisionSecond).isEqualTo(RetryDecision.RETRY);
    }
